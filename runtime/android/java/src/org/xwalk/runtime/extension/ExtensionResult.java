...
public class ExtensionResult {
    ...
    public ExtensionResult(Status status) {
        this(status, ExtensionResult.StatusMessages[status.ordinal()]);
    }

    public ExtensionResult(Status status, String message) {
        this.status = status.ordinal();
        this.messageType = message == null ? MESSAGE_TYPE_NULL : MESSAGE_TYPE_STRING;
        this.strMessage = message;
    }

    public ExtensionResult(Status status, JSONArray message) {
        this.status = status.ordinal();
        this.messageType = MESSAGE_TYPE_JSON;
        encodedMessage = message.toString();
    }

    public ExtensionResult(Status status, JSONObject message) {
        this.status = status.ordinal();
        this.messageType = MESSAGE_TYPE_JSON;
        encodedMessage = message.toString();
    }

    public ExtensionResult(Status status, int i) {
        this.status = status.ordinal();
        this.messageType = MESSAGE_TYPE_NUMBER;
        this.encodedMessage = ""+i;
    }

    public ExtensionResult(Status status, float f) {
        this.status = status.ordinal();
        this.messageType = MESSAGE_TYPE_NUMBER;
        this.encodedMessage = ""+f;
    }

    public ExtensionResult(Status status, boolean b) {
        this.status = status.ordinal();
        this.messageType = MESSAGE_TYPE_BOOLEAN;
        this.encodedMessage = Boolean.toString(b);
    }

    public ExtensionResult(Status status, byte[] data) {
        this(status, data, false);
    }

    public ExtensionResult(Status status, byte[] data, boolean binaryString) {
        this.status = status.ordinal();
    ...
    public String toCallbackString(String callbackId) {
        ...
        if ((status == ExtensionResult.Status.NO_RESULT.ordinal()) && keepCallback) {
        	return null;
        }

        // Check the success (OK, NO_RESULT & !KEEP_CALLBACK)
        if ((status == ExtensionResult.Status.OK.ordinal()) || (status == ExtensionResult.Status.NO_RESULT.ordinal())) {
